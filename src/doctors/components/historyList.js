...
class HistoryList extends Component {
  ...
  goToCancelAppointment = (item)=>{
    this.props.dispatch({
      type: 'SET_SELECTED_APPOINTMENT',
      payload: {
        selected_appointment_id: item._id,
      },
    });

    this.props.dispatch(
      NavigationActions.navigate({
        routeName: 'CancelAppointment',
      })
    );
  }
  ...
  renderItem = ({item}) => {
    return (
      <DoctorItem
        {...item}
        goToCancelAppointment={ ()=>{this.goToCancelAppointment(item)} }
        onPress={() => {
          this.goToDoctor(item);
        }}
      />
    );
  };
