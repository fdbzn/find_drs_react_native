...
class Api {
  userAppointment(userToken,paymentMethodId,device_session_id,appointmentId) {
    ...
    data_post = {paymentMethodId, device_session_id, appointmentId};
    console.log("enviados",data_post)
    return this.secure_post(`${BASE_API}payments`,data_post, userToken);
  }
  userRelativeAppointment(userToken, sourceId, appointmentId, relativeId) {
    return this.secure_post(
      `${BASE_API}payments`,
      {
        sourceId,
        appointmentId,
  ...
  addPaymentMethods(token_id, device_session_id, cvv, userToken) {
    return this.secure_post(
      `${BASE_API}users/me/paymentMethods`,
      {token_id, device_session_id, cvv},
      userToken
    );
  }
  ...
  getIntervalsByDate(workplace_id, date) {
    ...
    return this.get(
      `${BASE_API}workplaces/${workplace_id}/intervals?${params}`
    );
  }
  ...
  getDoctorById(doctorId) {
    return this.get(`${BASE_API}doctors/${doctorId}`);
  }
  ...
  getMyAppointments(token) {
    return this.secure_get(`${BASE_API}users/me/appointments/`, token);
  }
